...
/*  Пример запроса данных:
    ...
    Возвращает:   
       массив jбъектов с полями резалтсета или resultFields
...
function getDataFromTable_test() {
  ...
  return runGroupTests(
    {name: 'getDataFromTable',
     should: [1,1],
     data: [
       {sql: "SELECT idrubric, rubric FROM ref_rubric WHERE idrubric = ?",
        resultTypes: ['Int', 'String'],
        resultFields: ['id', 'rubric'],
        whereData: [1],
        whereTypes: ['Int']
       },
       { sql: "SELECT idrubric FROM ref_rubric WHERE idrubric = ?",
        resultTypes: ['Int'],
        whereData: [1],
        whereTypes: ['Int']
       }],
...
function startTransaction(session) {
  if (TEST("STOP_SQL_COMMIT")) return false;
...
//  types: ['String', 'Int', 'Blob'] - массив типов переменных для вставки
  ...
  try {    
    var stmt = connection.jdbc.prepareStatement(insert.sql, SQL_CONST('RETURN_GENERATED_KEYS'));
...
//    Примеря взят отсюда https://stackoverflow.com/questions/13543433/google-cloud-sql-return-generated-keys    
  ...
  var resultset = stmt.getGeneratedKeys(),
      ...
      i = 0;
  while(resultset.next()) { 
    result[i] = resultset.getInt(SQL_CONST('RETURN_GENERATED_KEYS'));
  }
  resultset.close();
  return result;
}
...
function saveCrawlerLog(log) {
  ...
  return doInsert({sql: 'INSERT INTO crawler_logger (date, source, obj, log, event) values (?,?,?,?,?)',
                   isCommit: true,
                   data: [dateToChar19 (), source, obj, message, log.event],
  ...
  return runGroupTests(
    {name: 'saveCrawlerLog',
     ...
     data: [{message: 'Тест работы функции saveCrawlerLog',
              ...
              event: LOG_EVENT('TEST')}
     ],
     compare: [
       '(result.count === pattern)'
     ],
     online: 'TEST("STOP_SQL_EXEC")',
     clean: 'doDelete({sql: "DELETE FROM crawler_logger WHERE event = '+LOG_EVENT('TEST')+'"})'
    });
}

function saveUserLog(log) {
  if(!log || !log.message) {
    Logger.log("[saveUserLog]: попытка сохранить пустое собщение");
    return {isOk: false,
            count: 0,
            error: '',
            generatedKey: 0};
  ...
  return doInsert({sql: 'INSERT INTO user_log (iduser, date, source, obj, log, event) values (?,?,?,?,?,?)',
                   isCommit: true,
                   data: [iduser, dateToChar19 (), source, obj, message, log.event],
                   types: ['Int', 'String', 'String', 'String', 'String', 'Int']});
}

function saveUserLog_test() {
  return runGroupTests(
    {name: 'saveUserLog',
     should: [1],
     data: [{message: 'Тест работы функции saveUserLog',
              obj: {test: {subtest: 'test'}},
              source: "saveCrawlerLog",
              iduser: "1",
              event: LOG_EVENT('TEST')}
     ...
     online: 'TEST("STOP_SQL_EXEC")',
     clean: 'doDelete({sql: "DELETE FROM user_log WHERE event = '+LOG_EVENT('TEST')+'"})'
    });
}
