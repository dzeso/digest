...
function saveParagraphs_test() {
  ...
  CacheService.getScriptCache().remove(CACHE('NEWS_ID_BY_KEY')+"1508776644");
    ...
    if (flUpdateNews) {
      ...
      if (updatedNews.isOk) {
        result = {
          ...
          code: CODE_RESULT("UPDATE_SUCCESSFULLY"),
          generatedKey: data.id };
        cleanCache(CACHE('NEWS_BY_ID') + data.id);
      }
      else result = {
        done: false,
        code: CODE_RESULT("UPDATE_FAILED"),
        generatedKey: null
      };
    } else {
      result = {
      ...
      cleanCache(CACHE('NEWS_BY_DATE') + news.dt.date);
    } 
  }
...
function saveNews_test() {
  ...
  data = mergeObjects([data,{
    ...
    link: 'CODE_RESULT("SAVE_SKIPED") = 301 так как дата редактирования позже даты апдейта',
    id: 1,
  ...
  var data2 = {};
    ...
    key: ('test_' + Date.parse(new Date()).toString()).substr(0,14),
    link: 'CODE_RESULT("UPDATE_SUCCESSFULLY") = 302 так как дата редактирования меньше даты апдейта',
  ...
  data1 = mergeObjects([data1,{
    key: ('test_' + Date.parse(new Date()).toString()).substr(0,14),
    link: 'CODE_RESULT("SAVE_SKIPED") = 301 так как нет даты апдейта',
    id: 1,
  ...
  data3.link = 'CODE_RESULT("SAVE_SUCCESSFULLY") = 300 - новая статья';
  data3.sourceId = RIA('SOURCE_ID')
  doDelete({sql: "DELETE FROM news WHERE key_from_source LIKE \'test_%\'"})
  return runGroupTests(
    {name: 'saveNews',
     should: [
       {done: true, code: CODE_RESULT("SAVE_SKIPED")},
       {done: true, code: CODE_RESULT("UPDATE_SUCCESSFULLY")},
       {done: true, code: CODE_RESULT("SAVE_SKIPED")},
       {done: true, code: CODE_RESULT("SAVE_SUCCESSFULLY")},
//       {done: false, code: CODE_RESULT("SAVE_FAILED")}
     ],
     data: [
       data, 
       data2, 
...
function saveNewRubricToRef(save) {  
  ...
  if (insert.isOk) {
    ...
    if (insert.isOk) {
      result.isOk = true; 
      setCacheId({key: CACHE('RUBRIC_BY_SYNONYM') + save.name, value: result.id});
    }
  }
  return result;
}
...
function saveNewsTags(save) {  
  ...
  for (var i = 0, len_i = save.tags.length; i < len_i; i++) {
    ...
    result = doInsert(
      {connection: save.connection,
       sql: 'INSERT INTO news_tags (idnews, idtag) values (?,?)',
...
function saveNewTagToRef(save) {  
  ...
  if (insert.isOk) {
    ...
    if (insert.isOk) {
      result.isOk = true; 
      setCacheId({key: CACHE('TAG_BY_SYNONYM') + save.tag.name, value: result.id});
    }
  }
  return result;
}
...
function doInsertTestNews_data(data) {
  ...
  var insertedNews = doInsert(
    {sql: 'INSERT INTO news (title, idsource, key_from_source, last_edited, date, time, link) values (?,?,?,?,?,?,?)',
     data: [title, RIA('SOURCE_ID'), key, date, date, time, link],
     types: ['String', 'Int', 'String', 'String', 'String', 'String', 'String']}); 
  return insertedNews;
}